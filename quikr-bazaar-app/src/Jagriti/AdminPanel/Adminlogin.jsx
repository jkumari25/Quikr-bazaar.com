  ...
  const handleSubmit = async() => {
    const isValidated = validatefirstname(firstname)&& validatelastname(lastname)&& validateemail(adminemail)&&validatePassword(adminpassword)
     ...
     if(isValidated){
      ...
      let res = await fetch('https://pear-gleaming-squid.cyclic.app/Adminlogin');
      ...
      if(result==false){
        let response=await fetch('https://pear-gleaming-squid.cyclic.app/Adminlogin',{
        method:'POST',
...
const Form2 = () => {
 ...
 const [show, setShow] = React.useState(false);
  ...
  let isLoading = useSelector((store) =>store.AdminLoginReducer.isLoading)
